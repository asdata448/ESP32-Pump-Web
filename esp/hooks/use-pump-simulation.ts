...
export function usePumpSimulation() {
  ...
  // Re-home - STOP PUMPING FIRST before rehoming
  const rehome = useCallback(() => {
    // If currently pumping, stop it first
    clearAllTimers()

    setState(prev => {
      // Only save to history if we were actually pumping
      if (prev.pumping && prev.stepsCompleted > 0) {
        const record: DemoHistoryRecord = {
          id: Date.now().toString(),
          syringeType: prev.syringeType,
          speedMlh: prev.speedMlh,
    ...
    // Start prepare sequence after a short delay
    setTimeout(() => {
      prepare()
    }, 100)
  }, [clearAllTimers, history, saveHistory, prepare])
